...
[![Version](https://img.shields.io/badge/version-2.6.0-blue)](https://github.com/it-09/ai-ml-scrapper)
...
| **Per job scraped** | $0.001 per job |

**Examples:**
- 100 jobs = $0.15 total ($0.05 start + $0.10 for jobs)
- 500 jobs = $0.55 total ($0.05 start + $0.50 for jobs)
- 1000 jobs = $1.05 total ($0.05 start + $1.00 for jobs)
...
- Ensure `maxItems` is within your tier limits (Free: 100, Starter: 500, Pro: 2000, Enterprise: 10000)
- If using AI enrichment, verify your API key environment variable is set

**No results returned:**
- Try running with default settings (empty keyword/company filters)
- Some sources may be temporarily unavailable — check logs for warnings
- Reduce `maxItems` to speed up the first run

**Timeout errors:**
- The Actor has a 4-minute runtime limit by default
- Reduce the number of sources or lower `maxItems` to complete faster
- Enable proxy if you're hitting rate limits from target sites

**Memory errors:**
- The Actor automatically manages memory usage
- For very large runs (>5000 jobs), consider running in batches
- Free tier uses 256MB; upgrade memory allocation in Apify Console if needed

**Webhook not firing:**
- Verify the webhook URL is accessible and accepts POST requests
- Check that `alertKeywords` or `alertMinSalary` are configured
- Ensure the webhook URL is not pointing to localhost or private IPs
...
- **v2.6.0** (2026-09): Memory streaming, progress logging, PPE pricing, safer defaults, reliability improvements
